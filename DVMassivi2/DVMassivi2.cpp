// DVMassivi2.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//

#include <iostream>
#include <ctime>
using namespace std;


void DynamicArray(int**& originalArr, int& sizeRow, int& sizeCol) // Выделение памяти под массив
{
    originalArr = new int* [sizeRow];
    for (int i = 0; i < sizeRow; i++) {
        originalArr[i] = new int[sizeCol];
    }
}
void FillingArray(int**& originalArr, int& sizeRow, int& sizeCol) // заполнение массива
{
    for (int i = 0; i < sizeRow; i++)
    {
        for (int j = 0; j < sizeCol; j++)
        {
            originalArr[i][j] = 1 + rand() % 9;
        }
    }
}

void PrintArray(int**& originalArr, int& sizeRow, int& sizeCol) // вывод массива на экран
{
    for (int i = 0; i < sizeRow; i++)
    {
        for (int j = 0; j < sizeCol; j++)
        {
            cout << originalArr[i][j] << " ";
        }
        cout << endl;
    }
}
void DelColumn(int**& originalArr, int**& arrDelColumn, int& sizeRow, int& sizeCol, int& position) // удаление столбца
{
    if (position - 1 != 0)
    {
        for (int i = 0; i < sizeRow; i++)
        {
            for (int j = 0; j < position - 1; j++)
            {
                arrDelColumn[i][j] = originalArr[i][j];
            }
        }
    }

    for (int i = 0; i < sizeRow; i++)
    {
        for (int j = position - 1; j < sizeCol; j++)
        {
            if (j != position - 1)
            {
                arrDelColumn[i][j - 1] = originalArr[i][j];
            }
        }
    }
}



int main()
{
    setlocale(0, "");
    srand(time(0));
    int sizeRow = 5;
    int sizeCol = 5;
    int sizeColDel = sizeCol - 1;
    int positionDelete = 0;
    int** originalArr;
    int** arrDelColumn;

    DynamicArray(originalArr, sizeRow, sizeCol);
    FillingArray(originalArr, sizeRow, sizeCol);

    cout << endl << endl;
    cout << "Изначальный массив: " << endl << endl;
    PrintArray(originalArr, sizeRow, sizeCol);
    DynamicArray(arrDelColumn, sizeRow, sizeColDel);
    cout << "Введите позицию для удаления столбца: "; 
    cin >> positionDelete; cout << endl;
    DelColumn(originalArr, arrDelColumn, sizeRow, sizeCol, positionDelete);
    cout << "Массив с удаленным столбцом в указанной позиции: " << endl << endl;
    PrintArray(arrDelColumn, sizeRow, sizeColDel);

    return 0;
}